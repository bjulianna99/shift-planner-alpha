...
function Main(){
  ...
  const builder=(this_1.h.push(new Elt("content", pageContent())),this_1);
...
function sampleShifts(){
  return _c.sampleShifts;
}
function renderShiftEntry(entry){
  return Doc.Element("div", [Attr.Create("class", "bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-4")], [Doc.Element("div", [Attr.Create("class", "flex flex-col md:flex-row md:items-center md:justify-between gap-3")], [Doc.Element("div", [], [Doc.Element("h3", [Attr.Create("class", "text-lg font-semibold text-gray-800")], [Doc.TextNode(DateFormatter(entry.Date, "yyyy-MM-dd"))]), Doc.Element("p", [Attr.Create("class", "text-gray-600 mt-1")], [Doc.TextNode(entry.Note)])]), Doc.Element("span", [Attr.Create("class", "inline-block px-3 py-1 rounded-full text-sm font-medium "+shiftTypeBadgeClass(entry.ShiftType))], [Doc.TextNode(shiftTypeToText(entry.ShiftType))])])]);
}
function shiftTypeBadgeClass(shiftType){
  return shiftType.$==1?"bg-purple-100 text-purple-700":shiftType.$==2?"bg-green-100 text-green-700":shiftType.$==3?"bg-yellow-100 text-yellow-700":"bg-blue-100 text-blue-700";
}
function shiftTypeToText(shiftType){
  return shiftType.$==1?"Night shift":shiftType.$==2?"Rest day":shiftType.$==3?"Leave":"Day shift";
}
function get(arr, n){
  checkBounds(arr, n);
  return arr[n];
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static sampleShifts;
  static {
    this.sampleShifts=ofArray([New(Create(2026, 4, 20, 0, 0, 0, 0), Day, "Regular day shift"), New(Create(2026, 4, 21, 0, 0, 0, 0), Night, "Night duty"), New(Create(2026, 4, 22, 0, 0, 0, 0), Rest, "Scheduled rest day"), New(Create(2026, 4, 23, 0, 0, 0, 0), Day, "Morning shift"), New(Create(2026, 4, 24, 0, 0, 0, 0), Leave, "Personal leave")]);
    this.pageContent=Doc.Element("div", [Attr.Create("class", "space-y-8")], [Doc.Element("section", [Attr.Create("id", "home")], [Doc.Element("div", [Attr.Create("class", "bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8")], [Doc.Element("h1", [Attr.Create("class", "text-3xl md:text-4xl font-bold text-gray-900 mb-4")], [Doc.TextNode("Shift Planner Alpha")]), Doc.Element("p", [Attr.Create("class", "text-lg text-gray-700 mb-4")], [Doc.TextNode("A simple F# WebSharper application for managing and viewing shift schedules.")]), Doc.Element("p", [Attr.Create("class", "text-gray-600")], [Doc.TextNode("This is the first project version with a custom layout and static sample shift entries.")])])]), Doc.Element("section", [Attr.Create("class", "grid md:grid-cols-3 gap-4")], [Doc.Element("div", [Attr.Create("class", "bg-blue-50 border border-blue-200 rounded-xl p-4")], [Doc.Element("h2", [Attr.Create("class", "text-lg font-semibold text-blue-800 mb-2")], [Doc.TextNode("Current stage")]), Doc.Element("p", [Attr.Create("class", "text-blue-700")], [Doc.TextNode("Initial project skeleton")])]), Doc.Element("div", [Attr.Create("class", "bg-white border border-gray-200 rounded-xl p-4")], [Doc.Element("h2", [Attr.Create("class", "text-lg font-semibold text-gray-800 mb-2")], [Doc.TextNode("Entries")]), Doc.Element("p", [Attr.Create("class", "text-gray-600")], [Doc.TextNode(String(sampleShifts().Length)+" sample shifts")])]), Doc.Element("div", [Attr.Create("class", "bg-white border border-gray-200 rounded-xl p-4")], [Doc.Element("h2", [Attr.Create("class", "text-lg font-semibold text-gray-800 mb-2")], [Doc.TextNode("Status")]), Doc.Element("p", [Attr.Create("class", "text-gray-600")], [Doc.TextNode("Static preview version")])])]), Doc.Element("section", [Attr.Create("id", "sample-shifts")], [Doc.Element("div", [Attr.Create("class", "flex items-center justify-between mb-4")], [Doc.Element("h2", [Attr.Create("class", "text-2xl font-semibold text-gray-800")], [Doc.TextNode("Sample shift entries")])]), Doc.Element("div", [], map(renderShiftEntry, sampleShifts()))]), Doc.Element("section", [Attr.Create("id", "about-project")], [Doc.Element("div", [Attr.Create("class", "bg-white rounded-2xl shadow-sm border border-gray-200 p-6")], [Doc.Element("h2", [Attr.Create("class", "text-2xl font-semibold text-gray-800 mb-3")], [Doc.TextNode("About this project")]), Doc.Element("p", [Attr.Create("class", "text-gray-700 mb-3")], [Doc.TextNode("Shift Planner Alpha is being developed step by step as a semester project.")]), Doc.Element("p", [Attr.Create("class", "text-gray-600")], [Doc.TextNode("Later versions will include adding shifts, deleting entries, sorting, summaries, and workload analysis.")])])])]);
  }
});
function Equals(a, b){
  if(a===b)return true;
...
function New(Date_1, ShiftType, Note){
  return{
    Date:Date_1, 
    ShiftType:ShiftType, 
    Note:Note
